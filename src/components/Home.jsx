    ...
    return (
        <div className="container">
                        ...
                        <div className="col-6 col-md-3 col-lg-3 mt-3 pt-3 pb-3" key={da.id}>
                            <div className="card p-2 shadow-sm">
                                ...
                                <Link to={`/product/${da.id}`} className="no-underline text-dark">
                                     
                                    <img src={da.image} className="img-fluid h-[200px] w-100 object-cover" alt={da.name || "Product"} />
                                </Link>
                                <center>
                                    <h3 className="d-flex justify-content-center align-items-center text-center text-success mt-2"> {da.title}</h3>
            ...
            <div className={cart ? "fixed h-full right-0 top-0 w-full md:w-[30%] bg-[#a0dd96] ease-in-out duration-500 z-50" : "fixed right-[-100%]"}>
                ...
                {/* Cart Items */}
                {/*
✅ MODIFICATION 1: We need to change the `div` with the class name "product"
to have a vertical scrollbar. 
We will add Tailwind CSS classes to manage its height and overflow.
 
The `h-full` class on the parent element is problematic because it makes the parent
container the same height as the viewport. The "Prakash" and "Cart Items" headers,
and the "Subtotal" footer take up space, which squeezes the content in the middle.
 
The best approach is to make the entire cart sidebar a flex container with a
column direction and let the product list grow and handle its own overflow.
 
Change the parent `div` to `flex flex-col h-full`
Change the content `div` to `flex-grow overflow-y-auto p-4`
*/}
                <div className="product flex-grow overflow-y-auto p-4">
                    {carts.length === 0 ? (
                        <p className="text-center mt-4">Your cart is empty</p>
                    ) : (
                        carts.map((car) => (
                            <div className="ab flex flex-wrap items-center justify-between border p-4 rounded-lg mt-6 mb-6" key={car.id}>
                                ...
                                <div className="flex flex-col ml-4 flex-grow">
                                    ...
                                    <h6 className="flex items-center">
                                        Qty : &nbsp;&nbsp;
                                        <button className="bg-green-700 text-white px-2" onClick={() => qtychange(car.id, car.quantity - 1)}>
                                            -
                                        </button>
                                        <input type="number" name="qty" value={car.quantity} onChange={(e) => qtychange(car.id, e.target.value)} className="bg-white w-[50px] text-center mx-1" />
                                        <button className="bg-green-700 text-white px-2" onClick={() => qtychange(car.id, car.quantity + 1)}>
                                            +
                                        </button>
                                    </h6>
                                    ...
                                    <p className="flex items-center cursor-pointer text-red-500" onClick={() => handleDel(car.id)}>
                                        ...
                                        <span className="ml-2" onClick={() => handleDel(car.id)}>
                                            Remove Item
                                        </span>
                                    </p>
                                </div>
